...
from openapi_server.peripherals.db import gaia_db_query, gaia_radecs
...
def generate_image(exp_time, light = 1, seeing=config['CAMERA']['seeing']):
    ...
    if light == 1:
        ...
        fovy = camera.numy * camera.plate_scale / 3600

        if config['LOCAL_DB']:
            # only tmass stars. db has mags of gaia passbands, but don't have conversion to photons.
            gaias, mags = gaia_db_query(center, (fovx, fovy), tmass=True, dateobs=dateobs, 
                                        db_path=config['LOCAL_DB_PATH'])
        else:
            if config['TMASS']:
                gaias, mags = gaia_radecs(center, (fovx*1.5, fovy*1.5), tmass=True, dateobs=dateobs)
            else:
                # gives phot_rp_mean_flux in [e-/s] instead of J magnitude
                gaias, fluxes = gaia_radecs(center, (fovx*1.5, fovy*1.5), tmass=False, dateobs=dateobs)

        if config['TMASS'] or config['LOCAL_DB']:
            ...
            fluxes = photons * 10**(-0.4*mags) * camera.qe * camera.collecting_area * exp_time # [electrons]
        else:
            fluxes = fluxes * camera.qe * camera.collecting_area * exp_time # [electrons]
        ...
        base = np.ones((camera.numy, camera.numx)) * (camera.darkcurrent + camera.skybackground) * exp_time # [electrons pixel^-1]

        # add stars to base, with fluxes as brightness
        for i in range(len(gaias_pixel[0])):
            try:
                base[int(gaias_pixel[1][i]), int(gaias_pixel[0][i])] += fluxes[i]
            except:
                pass

        # convolve with gaussian to simulate seeing
        image = gaussian_filter(base, sigma=int((seeing/camera.plate_scale)/2.355)).astype(np.float64)

        # add poisson noise based on signal
        image = np.random.poisson(image).astype(np.float64)

        # add read noise
        image += np.random.normal(0, camera.readnoise, (camera.numy, camera.numx)).astype(np.float64)
        ...
        if config['FLATS']:
            ...
            sun_altaz = sun_position.transform_to(AltAz(obstime=obs_time, location=obs_location))
            ...
            flux_sky = (camera.plate_scale)**2 * 100 * 10**(0.415 * sun_alt + 5.926) * camera.qe * camera.collecting_area * exp_time # [electrons pixel^-1 ]
        ...
        image = np.ones((camera.numy, camera.numx)) * (camera.darkcurrent) * exp_time # [electrons pixel^-1]
...
async def binx_get(
    ...
    return IntResponse(Value=camera.binx, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def biny_get(
    ...
    return IntResponse(Value=camera.biny, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def camerastate_get(
    ...
    return IntResponse(Value=camera.state, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def cameraxsize_get(
    ...
    return IntResponse(Value=camera.width, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def cameraysize_get(
    ...
    return IntResponse(Value=camera.height, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def ccdtemperature_get(
    ...
    return DoubleResponse(Value=camera.temperature, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def cooleron_get(
    ...
    return BoolResponse(Value=camera.cooleron, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def imagearray_get(
    ...
    return StreamingResponse(bytes_generator(camera.imagearray), headers=headers, media_type='application/imagebytes')  
...
async def imageready_get(
    ...
    if (datetime.utcnow() - camera.lastexposurestarttime).total_seconds() > camera.lastexposureduration:
        camera.imageready = True
        camera.state = 0
    else:
        camera.imageready = False

    return BoolResponse(Value=camera.imageready, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def lastexposurestarttime_get(
    ...
    return StringResponse(Value=str(camera.lastexposurestarttime), ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def numx_get(
    ...
    return IntResponse(Value=camera.numx, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def numx_put(
    device_number: int = Path(..., description="Zero based device number as set on the server (0 to 4294967295)", ge=0, le=4294967295),
    NumX: int = Form(0, description="Sets the subframe width, if binning is active, value is in binned pixels."),
    ClientID: int = Form(None, description="Client's unique ID. (0 to 4294967295). The client should choose a value at start-up, e.g. a random value between 0 and 65535, and send this value on every transaction to help associate entries in device logs with this particular client."),
    ClientTransactionID: int = Form(None, description="Client's transaction ID. (0 to 4294967295). The client should start this count at 1 and increment by one on each successive transaction. This will aid associating entries in device logs with corresponding entries in client side logs."),
) -> MethodResponse:
...
async def numy_get(
    ...
    return IntResponse(Value=camera.numy, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def numy_put(
    device_number: int = Path(..., description="Zero based device number as set on the server (0 to 4294967295)", ge=0, le=4294967295),
    NumY: int = Form(0, description="Sets the subframe height, if binning is active, value is in binned pixels."),
    ClientID: int = Form(None, description="Client's unique ID. (0 to 4294967295). The client should choose a value at start-up, e.g. a random value between 0 and 65535, and send this value on every transaction to help associate entries in device logs with this particular client."),
    ClientTransactionID: int = Form(None, description="Client's transaction ID. (0 to 4294967295). The client should start this count at 1 and increment by one on each successive transaction. This will aid associating entries in device logs with corresponding entries in client side logs."),
) -> MethodResponse:
...
async def pixelsizex_get(
    ...
    return DoubleResponse(Value=camera.pitch, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def sensorname_get(
    ...
    return StringResponse(Value=camera.sensor_name, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def sensortype_get(
    ...
    return IntResponse(Value=0, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def cansetccdtemperature_get(
    ...
    return BoolResponse(Value=True, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def setccdtemperature_put(
    device_number: int = Path(..., description="Zero based device number as set on the server (0 to 4294967295)", ge=0, le=4294967295),
    SetCCDTemperature: float = Form(-10, description="Temperature set point (degrees Celsius)."),
    ClientID: int = Form(None, description="Client's unique ID. (0 to 4294967295). The client should choose a value at start-up, e.g. a random value between 0 and 65535, and send this value on every transaction to help associate entries in device logs with this particular client."),
    ClientTransactionID: int = Form(None, description="Client's transaction ID. (0 to 4294967295). The client should start this count at 1 and increment by one on each successive transaction. This will aid associating entries in device logs with corresponding entries in client side logs."),
) -> MethodResponse:
...
async def startx_get(
    ...
    return IntResponse(Value=camera.startx, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def startx_put(
    device_number: int = Path(..., description="Zero based device number as set on the server (0 to 4294967295)", ge=0, le=4294967295),
    StartX: int = Form(0, description="The subframe X axis start position in binned pixels."),
    ClientID: int = Form(None, description="Client's unique ID. (0 to 4294967295). The client should choose a value at start-up, e.g. a random value between 0 and 65535, and send this value on every transaction to help associate entries in device logs with this particular client."),
    ClientTransactionID: int = Form(None, description="Client's transaction ID. (0 to 4294967295). The client should start this count at 1 and increment by one on each successive transaction. This will aid associating entries in device logs with corresponding entries in client side logs."),
) -> MethodResponse:
...
async def starty_get(
    ...
    return IntResponse(Value=camera.starty, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def starty_put(
    device_number: int = Path(..., description="Zero based device number as set on the server (0 to 4294967295)", ge=0, le=4294967295),
    StartY: int = Form(0, description="The subframe Y axis start position in binned pixels."),
    ClientID: int = Form(None, description="Client's unique ID. (0 to 4294967295). The client should choose a value at start-up, e.g. a random value between 0 and 65535, and send this value on every transaction to help associate entries in device logs with this particular client."),
    ClientTransactionID: int = Form(None, description="Client's transaction ID. (0 to 4294967295). The client should start this count at 1 and increment by one on each successive transaction. This will aid associating entries in device logs with corresponding entries in client side logs."),
) -> MethodResponse:
...
async def canasymmetricbin_get(
    ...
    return BoolResponse(Value=False, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def electronsperadu_get(
    ...
    return DoubleResponse(Value=camera.gain, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def fullwellcapacity_get(
    ...
    return DoubleResponse(Value=camera.welldepth, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def gain_put(
    device_number: int = Path(..., description="Zero based device number as set on the server (0 to 4294967295)", ge=0, le=4294967295),
    Gain: int = Form(0, description="Index of the current camera gain in the Gains string array."),
    ClientID: int = Form(None, description="Client's unique ID. (0 to 4294967295). The client should choose a value at start-up, e.g. a random value between 0 and 65535, and send this value on every transaction to help associate entries in device logs with this particular client."),
    ClientTransactionID: int = Form(None, description="Client's transaction ID. (0 to 4294967295). The client should start this count at 1 and increment by one on each successive transaction. This will aid associating entries in device logs with corresponding entries in client side logs."),
) -> MethodResponse:
    """The camera's gain (GAIN VALUE MODE) OR the index of the selected camera gain description in the Gains array (GAINS INDEX MODE)."""
...
async def maxadu_get(
    ...
    return IntResponse(Value=camera.maxadu, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def maxbinx_get(
    ...
    return IntResponse(Value=1, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def lastexposureduration_get(
    ...
    return DoubleResponse(Value=camera.lastexposureduration, ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def offset_put(
    device_number: int = Path(..., description="Zero based device number as set on the server (0 to 4294967295)", ge=0, le=4294967295),
    Offset: int = Form(0, description="Index of the current camera offset in the offsets string array."),
    ClientID: int = Form(None, description="Client's unique ID. (0 to 4294967295). The client should choose a value at start-up, e.g. a random value between 0 and 65535, and send this value on every transaction to help associate entries in device logs with this particular client."),
    ClientTransactionID: int = Form(None, description="Client's transaction ID. (0 to 4294967295). The client should start this count at 1 and increment by one on each successive transaction. This will aid associating entries in device logs with corresponding entries in client side logs."),
) -> MethodResponse:
...
async def pulseguide_put(
    device_number: int = Path(..., description="Zero based device number as set on the server (0 to 4294967295)", ge=0, le=4294967295),
    Direction: int = Form(None, description="Direction of movement (0 &#x3D; North, 1 &#x3D; South, 2 &#x3D; East, 3 &#x3D; West)"),
...
async def readoutmode_put(
    device_number: int = Path(..., description="Zero based device number as set on the server (0 to 4294967295)", ge=0, le=4294967295),
    ReadoutMode: int = Form(0, description="Index into the ReadoutModes array of string readout mode names indicating the camera's current readout mode."),
    ClientID: int = Form(None, description="Client's unique ID. (0 to 4294967295). The client should choose a value at start-up, e.g. a random value between 0 and 65535, and send this value on every transaction to help associate entries in device logs with this particular client."),
    ClientTransactionID: int = Form(None, description="Client's transaction ID. (0 to 4294967295). The client should start this count at 1 and increment by one on each successive transaction. This will aid associating entries in device logs with corresponding entries in client side logs."),
) -> MethodResponse:
...
async def readoutmodes_get(
    ...
    if camera.device_number != device_number:
        raise HTTPException(status_code=404, detail="Device not found.")
    return StringArrayResponse(Value=["no"], ClientTransactionID=ClientTransactionID, ServerTransactionID=ClientTransactionID, ErrorNumber=0, ErrorMessage="")
...
async def subexposureduration_put(
    device_number: int = Path(..., description="Zero based device number as set on the server (0 to 4294967295)", ge=0, le=4294967295),
    SubExposureDuration: float = Form(0, description="The request sub exposure duration in seconds"),
    ClientID: int = Form(None, description="Client's unique ID. (0 to 4294967295). The client should choose a value at start-up, e.g. a random value between 0 and 65535, and send this value on every transaction to help associate entries in device logs with this particular client."),
    ClientTransactionID: int = Form(None, description="Client's transaction ID. (0 to 4294967295). The client should start this count at 1 and increment by one on each successive transaction. This will aid associating entries in device logs with corresponding entries in client side logs."),
) -> MethodResponse:
